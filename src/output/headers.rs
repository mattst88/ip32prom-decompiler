...
use crate::hardware::memmap::{
    ARCS_SPB, ARCS_SPB_OFFSET_ADAPTER_COUNT, ARCS_SPB_OFFSET_DEBUG_BLOCK,
    ARCS_SPB_OFFSET_FIRMWARE_VECTOR, ARCS_SPB_OFFSET_FIRMWARE_VECTOR_LENGTH,
    ARCS_SPB_OFFSET_GE_VECTOR, ARCS_SPB_OFFSET_LENGTH, ARCS_SPB_OFFSET_PRIVATE_VECTOR,
    ARCS_SPB_OFFSET_PRIVATE_VECTOR_LENGTH, ARCS_SPB_OFFSET_RESTART_BLOCK, ARCS_SPB_OFFSET_REVISION,
    ARCS_SPB_OFFSET_SIGNATURE, ARCS_SPB_OFFSET_UTLB_MISS_VECTOR, ARCS_SPB_OFFSET_VERSION,
    BASE_AUDIO, BASE_CRIME, BASE_I2C, BASE_ISA, BASE_KBD_MS, BASE_MACE_PCI, BASE_RENDER, BASE_RTC,
    FIRMWARE_VECTOR, FV_OFFSET_ADD_CHILD, FV_OFFSET_CLOSE, FV_OFFSET_DELETE_COMPONENT,
    FV_OFFSET_ENTER_INTERACTIVE_MODE, FV_OFFSET_EXECUTE, FV_OFFSET_FLUSH_ALL_CACHES,
    FV_OFFSET_GET_CHILD, FV_OFFSET_GET_COMPONENT, FV_OFFSET_GET_CONFIGURATION_DATA,
    FV_OFFSET_GET_DIRECTORY_ENTRY, FV_OFFSET_GET_DISPLAY_STATUS,
    FV_OFFSET_GET_ENVIRONMENT_VARIABLE, FV_OFFSET_GET_FILE_INFORMATION,
    FV_OFFSET_GET_MEMORY_DESCRIPTOR, FV_OFFSET_GET_PARENT, FV_OFFSET_GET_PEER,
    FV_OFFSET_GET_READ_STATUS, FV_OFFSET_GET_RELATIVE_TIME, FV_OFFSET_GET_SYSTEM_ID,
    FV_OFFSET_GET_TIME, FV_OFFSET_HALT, FV_OFFSET_INVOKE, FV_OFFSET_LOAD, FV_OFFSET_MOUNT,
    FV_OFFSET_OPEN, FV_OFFSET_POWER_DOWN, FV_OFFSET_READ, FV_OFFSET_REBOOT, FV_OFFSET_RESTART,
    FV_OFFSET_SAVE_CONFIGURATION, FV_OFFSET_SEEK, FV_OFFSET_SET_ENVIRONMENT_VARIABLE,
    FV_OFFSET_SET_FILE_INFORMATION, FV_OFFSET_TEST_UNICODE_CHARACTER, FV_OFFSET_WRITE, KSEG0,
    ...
    PRIVATE_VECTOR, ROM_ALIGN, ROM_SIZE, RTSB_OFFSET_BOOT_MASTER_ID, RTSB_OFFSET_BOOT_STATUS,
    RTSB_OFFSET_CHECKSUM, RTSB_OFFSET_LENGTH, RTSB_OFFSET_NEXT_RSTB, RTSB_OFFSET_PROCESSOR_ID,
    RTSB_OFFSET_RESTART_ADDRESS, RTSB_OFFSET_REVISION, RTSB_OFFSET_SAVE_AREA_LENGTH,
    RTSB_OFFSET_SAVED_STATE_AREA, RTSB_OFFSET_SIGNATURE, RTSB_OFFSET_VERSION,
};
...
fn emit_low_memory_areas(file: &mut File) -> Result<()> {
    ...
    writeln!(file)?;
    emit_define_hex!(file, FIRMWARE_VECTOR, "Firmware vector table")?;
    emit_define_hex!(file, PRIVATE_VECTOR, "Private vector table")?;
    writeln!(file)?;
    emit_define_hex!(file, FV_OFFSET_LOAD)?;
    emit_define_hex!(file, FV_OFFSET_INVOKE)?;
    emit_define_hex!(file, FV_OFFSET_EXECUTE)?;
    emit_define_hex!(file, FV_OFFSET_HALT)?;
    emit_define_hex!(file, FV_OFFSET_POWER_DOWN)?;
    emit_define_hex!(file, FV_OFFSET_RESTART)?;
    emit_define_hex!(file, FV_OFFSET_REBOOT)?;
    emit_define_hex!(file, FV_OFFSET_ENTER_INTERACTIVE_MODE)?;
    emit_define_hex!(file, FV_OFFSET_GET_PEER)?;
    emit_define_hex!(file, FV_OFFSET_GET_CHILD)?;
    emit_define_hex!(file, FV_OFFSET_GET_PARENT)?;
    emit_define_hex!(file, FV_OFFSET_GET_CONFIGURATION_DATA)?;
    emit_define_hex!(file, FV_OFFSET_ADD_CHILD)?;
    emit_define_hex!(file, FV_OFFSET_DELETE_COMPONENT)?;
    emit_define_hex!(file, FV_OFFSET_GET_COMPONENT)?;
    emit_define_hex!(file, FV_OFFSET_SAVE_CONFIGURATION)?;
    emit_define_hex!(file, FV_OFFSET_GET_SYSTEM_ID)?;
    emit_define_hex!(file, FV_OFFSET_GET_MEMORY_DESCRIPTOR)?;
    emit_define_hex!(file, FV_OFFSET_GET_TIME)?;
    emit_define_hex!(file, FV_OFFSET_GET_RELATIVE_TIME)?;
    emit_define_hex!(file, FV_OFFSET_GET_DIRECTORY_ENTRY)?;
    emit_define_hex!(file, FV_OFFSET_OPEN)?;
    emit_define_hex!(file, FV_OFFSET_CLOSE)?;
    emit_define_hex!(file, FV_OFFSET_READ)?;
    emit_define_hex!(file, FV_OFFSET_GET_READ_STATUS)?;
    emit_define_hex!(file, FV_OFFSET_WRITE)?;
    emit_define_hex!(file, FV_OFFSET_SEEK)?;
    emit_define_hex!(file, FV_OFFSET_MOUNT)?;
    emit_define_hex!(file, FV_OFFSET_GET_ENVIRONMENT_VARIABLE)?;
    emit_define_hex!(file, FV_OFFSET_SET_ENVIRONMENT_VARIABLE)?;
    emit_define_hex!(file, FV_OFFSET_GET_FILE_INFORMATION)?;
    emit_define_hex!(file, FV_OFFSET_SET_FILE_INFORMATION)?;
    emit_define_hex!(file, FV_OFFSET_FLUSH_ALL_CACHES)?;
    emit_define_hex!(file, FV_OFFSET_TEST_UNICODE_CHARACTER)?;
    emit_define_hex!(file, FV_OFFSET_GET_DISPLAY_STATUS)?;
    writeln!(file)?;
    Ok(())
}
